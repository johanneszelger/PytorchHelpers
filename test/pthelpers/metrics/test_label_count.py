...
from src.metrics.label_count import LabelCount
...
class LabelCountMetric(unittest.TestCase):
    ...
    def test_update_1hot(self):
        self.update(torch.tensor([[0, 1, 1, 0, 0]]))

        self.assertEqual(0, self.metric0.compute().item())
        self.assertEqual(1, self.metric1.compute().item())
        self.assertEqual(1, self.metric2.compute().item())
        self.assertEqual(0, self.metric3.compute().item())
        self.assertEqual(0, self.metric4.compute().item())

        self.update(torch.tensor([[0, 0, 1, 1, 0], [1, 0, 1, 1, 0]]))

        self.assertEqual(1, self.metric0.compute().item())
        self.assertEqual(1, self.metric1.compute().item())
        self.assertEqual(3, self.metric2.compute().item())
        self.assertEqual(2, self.metric3.compute().item())
        self.assertEqual(0, self.metric4.compute().item())
